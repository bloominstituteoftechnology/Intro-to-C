...
char *test_swap()
{
    ...
    mu_assert((ints_short[0] == 9 && ints_short[1] == 4), "weird Your swap function did not swap values correctly.");

    int ints_long[] = { 1, 9, 2, 8, 3, 7, 6, 4, 5 };
    swap(&ints_long[0], &ints_long[8]);
    swap(&ints_long[3], &ints_long[4]);
    mu_assert((ints_long[0] == 5 && ints_long[8] == 1), "dude Your swap function did not swap values correctly.");
    mu_assert((ints_long[3] == 3 && ints_long[4] == 8), "ahhh Your swap function did not swap values correctly.");
