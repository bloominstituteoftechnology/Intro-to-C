...
{
    struct Person *allocate_person = malloc(sizeof(struct Person));

    allocate_person -> name = string_dup(name);
    allocate_person -> age = age;
    allocate_person -> height = height;
    allocate_person -> weight = weight;
    
    return allocate_person;
}
