...
{
    struct Person *newPeep = malloc(sizeof(struct Person));
    newPeep->name = string_dup(name);
    newPeep->age =age;
    newPeep->height= height;
    newPeep->weight = weight;
    return newPeep;
}
...
*/
void Person_destroy(struct Person *newPeep)
{
free(newPeep->name);
free(newPeep);
}
