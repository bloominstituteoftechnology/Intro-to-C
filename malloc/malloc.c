...
{
    char *result = dest;        // result is set to the dsestination
    char *input = src;          // input is set to source
    for (int i = 0; i < n; i++) // iterates until index is equal to n or greater
    {
        result[i] = input[i]; // result is saved as input at index every iteration
    }
    return result; // returns the result of the for loop
}
